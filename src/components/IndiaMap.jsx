...
import { ArrowRight, RotateCcw, Users, MapPinned, Zap } from "lucide-react";

// Accurate traced outline of India (not the old hand-drawn stylized blob),
// sourced from a public-domain silhouette trace. Cities below are plotted
// by real latitude/longitude onto this same 1024x1024 space (equirectangular
// approximation — accurate enough for a node-network illustration, not
// cartographic use), so a city's dot actually sits where that city is.
const INDIA_PATH =
...
const CITIES = [
  { id: "gurugram", name: "Gurugram (HQ)", state: "Delhi-NCR, Haryana", x: 316, y: 330, fact: "Delhi-NCR headquarters — senior advisory team & doorstep documentation origin." },
  { id: "delhi", name: "New Delhi", state: "National Capital", x: 330, y: 316, fact: "Fastest-growing NBFC lending corridor — 48hr average sanction." },
  { id: "chandigarh", name: "Chandigarh", state: "Punjab & Haryana", x: 316, y: 241, fact: "North-India corporate & business loan advisory hub." },
  { id: "jaipur", name: "Jaipur", state: "Rajasthan", x: 283, y: 375, fact: "Rajasthan's fastest-growing LAP and business loan corridor." },
  { id: "lucknow", name: "Lucknow", state: "Uttar Pradesh", x: 453, y: 377, fact: "UP's leading education & personal loan disbursal center." },
  { id: "patna", name: "Patna", state: "Bihar", x: 591, y: 421, fact: "Eastern-belt MSME lending expansion, doorstep-first model." },
  { id: "guwahati", name: "Guwahati", state: "Assam", x: 808, y: 402, fact: "Northeast gateway — digital-first advisory, zero branch overhead." },
  { id: "ahmedabad", name: "Ahmedabad", state: "Gujarat", x: 177, y: 511, fact: "Gujarat's SME and machinery financing stronghold." },
  { id: "kolkata", name: "Kolkata", state: "West Bengal", x: 697, y: 527, fact: "Eastern-region doorstep advisory coverage, launched 2025." },
  { id: "mumbai", name: "Mumbai", state: "Maharashtra", x: 187, y: 650, fact: "Highest Home Loan disbursal volume — 8.40% onwards on prime profiles." },
  { id: "pune", name: "Pune", state: "Maharashtra", x: 220, y: 669, fact: "IT & manufacturing corridor for fast-track personal loans." },
  { id: "hyderabad", name: "Hyderabad", state: "Telangana", x: 372, y: 709, fact: "Education loan corridor for domestic & international study." },
  { id: "bengaluru", name: "Bengaluru", state: "Karnataka", x: 342, y: 864, fact: "Tech-sector Personal Loan approvals average 24 hours flat." },
  { id: "chennai", name: "Chennai", state: "Tamil Nadu", x: 431, y: 860, fact: "MSME & machinery loan corridor for southern manufacturing belt." },
  { id: "kochi", name: "Kochi", state: "Kerala", x: 299, y: 970, fact: "Kerala's NRI and home loan remittance-linked lending hub." },
];
...
export default function IndiaMap() {
  const [active, setActive] = useState("gurugram");
  ...
  return (
    <div className="flip-card-scene">
      <div className="flip-card-inner" data-flipped={flipped}>
        {/* Front: the map. Only the explicit "Why no branch offices?" button
            below is keyboard-focusable for the flip — this outer div is a
            mouse-only click-to-flip convenience (per design), deliberately
            not given a role/tabIndex/onKeyDown of its own, since a
            keydown handler here would otherwise catch Enter/Space presses
            bubbling up from the city dots and pills nested inside it and
            flip the card when the user meant to just pick a city. */}
        <div
          className="flip-card-face bg-teal-dark rounded-2xl p-6 sm:p-8 cursor-pointer"
          onClick={() => setFlipped(true)}
        >
          <div className="grid md:grid-cols-5 gap-6 items-center">
            <div className="md:col-span-3 flex justify-center">
              <svg viewBox="0 0 1024 1024" className="w-full max-w-xs" role="img" aria-label="Map of India with Aaricca coverage cities">
                <g transform="translate(0,1024) scale(0.1,-0.1)" fill="#ffffff0d" stroke="#ffffff2e" strokeWidth="8">
                  <path d={INDIA_PATH} />
                </g>
                {CITIES.map((c) => (
                  <g
                    key={c.id}
                    onClick={(e) => selectCity(c.id, e)}
                    onKeyDown={(e) => {
                      if (e.key !== "Enter" && e.key !== " ") return;
                      e.preventDefault(); // Space must select the city, not scroll the page.
                      selectCity(c.id, e);
                    }}
            ...
            <div className="md:col-span-2 text-white">
              ...
              <button
                ...
                className="mt-5 flex items-center gap-1.5 text-xs font-semibold text-gold hover:text-gold-dark transition-colors"
              >
                Why no branch offices? <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          </div>
        </div>

        {/* Back: why no physical branches */}
        <div className="flip-card-face flip-card-face-back bg-teal-dark rounded-2xl p-6 sm:p-8 flex flex-col justify-center">
          <p className="text-xs uppercase tracking-wide text-gold font-semibold mb-2">One HQ. Zero borders.</p>
          <h3 className="font-display font-bold text-xl text-white mb-3">
            Every city on that map is served from one desk at our Delhi-NCR head office.
          </h3>
          <p className="text-sm text-cream/80 leading-relaxed mb-5">
            We don't run 100+ branch offices, and we never will. Your application is reviewed personally by our
            senior team of financial experts at our Delhi-NCR head office (Gurugram) — then executed locally through
            vetted on-ground partners in 120+ cities for documentation pickup and disbursal. Same advisor, start to
            finish, no matter which city you're calling from.
          </p>
          <ul className="space-y-2.5 mb-6">
            {WHY_POINTS.map((p) => (
              <li key={p.text} className="flex items-start gap-2.5 text-sm text-cream/80">
                <p.icon className="w-4 h-4 text-gold shrink-0 mt-0.5" />
                {p.text}
              </li>
            ))}
          </ul>
          <button
            ...
            className="self-start flex items-center gap-1.5 text-xs font-semibold text-gold hover:text-gold-dark transition-colors"
          >
            <RotateCcw className="w-3.5 h-3.5" /> Back to the map
          </button>
        </div>
      </div>
    </div>
